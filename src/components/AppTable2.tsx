...
import { MoreVert, Search } from "@nine-thirty-five/material-symbols-react/rounded";
import { ArrowRightAlt, CheckCircle } from "@nine-thirty-five/material-symbols-react/outlined";
...
function getStatusLabel(row: RequestedQuotationRow) {
    return toTitleCase(row.assignment_status ?? row.status);
}

function getStatusClassName(row: RequestedQuotationRow) {
    if (row.assignment_status?.toUpperCase().includes("REASSIGN")) {
        return classes.statusReassign;
    }

    if (row.status.toUpperCase() === "ACCEPTED") {
                            ...
                            quotations.map((row) => (
                                ...
                                >
                                    <Table.Td className={classes.cellRequest}>
                                        <div className={classes.requestCode}>{row.reference_number}</div>
                                        <div className={classes.subText}>{row.client_full_name}</div>
                                        <div className={classes.subText}>Req. Date: {row.date}</div>
                                    </Table.Td>

                                    <Table.Td className={classes.cellDetails}>
                                        <div className={classes.detailTitle}>{getServiceLabel(row.service)}</div>
                                        {row.logistics_service ? (
                                            <>
                                                <div className={classes.subText}>{row.logistics_service.commodity}</div>
                                                <div className={classes.subText}>
                                                    {toTitleCase(row.logistics_service.service_type)} · {toTitleCase(row.logistics_service.transport_mode)}
                                                </div>
                                                <div className={classes.routeLine}>
                                                    <span className={classes.subText}>{row.logistics_service.origin}</span>
                                                    <ArrowRightAlt width={15} />
                                                    <span className={classes.subText}>{row.logistics_service.destination}</span>
                                                </div>
                                            </>
                                    ...
                                    <Table.Td className={classes.cellStatus}>
                                        <div className={`${classes.statusTag} ${getStatusClassName(row)}`}>
                                            <CheckCircle width={16} /> {getStatusLabel(row)}
                                        </div>
                                        {row.as_full_name && (
                                            <div className={classes.statusOwner}>
                                                {row.as_full_name}
                                                {row.as_username ? ` (@${row.as_username})` : ""}
                                            </div>
                                        )}
                                        {row.assignment_status && (
                                            <div className={`${classes.statusOwner} ${classes.statusNote}`}>
                                                {toTitleCase(row.assignment_status)}
                                            </div>
                                        )}
                                        {row.assigned_at && (
                                            <div className={classes.statusOwner}>Assigned at: {row.assigned_at}</div>
                                        )}
                                        {row.prepared_by && (
                                            <div className={classes.statusOwner}>Prepared by: {row.prepared_by}</div>
                                        )}
                                    </Table.Td>
